...
[colN,rowN] = meshgrid(1:size(cam1H,2),1:size(cam1H,1));
colN_v = reshape(colN,size(ref_row,1)*size(ref_row,2),1);
rowN_v = reshape(rowN,size(ref_row,1)*size(ref_row,2),1);
ref_col_v = round(ref_col_v,4);
ref_row_v = round(ref_row_v,4);
...
pert_col_v = round(pert_col_v,4);
pert_row_v = round(pert_row_v,4);
